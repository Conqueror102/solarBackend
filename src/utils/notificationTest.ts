...
import { 
    ...
    createProductRestockNotification,
...
export const testNotificationSystem = async () => {
    try {
        ...
        const systemNotification = await createSystemNotification(
            userId,
            'Welcome to Solar!',
            'Thank you for joining our platform. We hope you enjoy your shopping experience.',
            { welcome: true }
        );
        ...
        const orderNotification = await createOrderStatusNotification(
            userId,
            'order_123456',
            'shipped',
            {
        ...
        const paymentNotification = await createPaymentSuccessNotification(
            userId,
            'order_123456',
            99.99,
            'credit_card'
        ...
        const restockNotification = await createProductRestockNotification(
            userId,
            'product_123',
            'Solar Panel 100W'
        );
        ...
        const promotionNotification = await createPromotionNotification(
            userId,
            'Holiday Sale!',
            'Get up to 50% off on all solar products this holiday season.',
            { saleId: 'holiday2024', discount: 50 }
        );
        ...
        const allUsersNotification = await createNotificationForAllUsers(
            'system',
            'System Maintenance',
            'Our platform will be under maintenance from 2:00 AM to 4:00 AM UTC.',
            { maintenance: true, startTime: '2024-01-01T02:00:00Z', endTime: '2024-01-01T04:00:00Z' },
            'high'
        );
        ...
        const adminNotification = await createNotificationForRole(
            'admin',
            'system',
            'Admin Alert',
            'New user registration requires approval.',
            { alert: true, type: 'user_registration' },
            'high'
        );
