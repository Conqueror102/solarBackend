...
export const { worker: emailWorker } = makeWorker(
  'email',
  async (job) => {
    switch (job.name) {
      case 'orderPlaced': {
        const d = job.data as { to:string; userName?:string; orderId:string; totalAmount:number; status:string };
        await sendOrderPlacedEmail({ email: d.to, name: d.userName }, { _id: d.orderId, totalAmount: d.totalAmount, status: d.status });
        return { sent: true };
      }
      case 'orderStatus': {
        const d = job.data as { to:string; userName?:string; orderId:string; totalAmount:number; status:string; paymentStatus?:string };
      ...
      case 'authEmailVerification': {
        const d = job.data as { to:string; userName?:string; token:string; frontendUrl:string };
