...
const handleDuplicateKeyError = (err: any) => {
    ...
    return new AppError(
        SAFE_ERROR_MESSAGES.EMAIL_IN_USE,
        409,
        'DUPLICATE_ERROR'
    );
};
...
const handleJoiValidationError = (err: Joi.ValidationError) => {
    return new AppError(
        SAFE_ERROR_MESSAGES.VALIDATION_FAILED,
        400,
        'VALIDATION_ERROR'
    );
};
...
const handleJWTError = () => {
    return new AppError(
        SAFE_ERROR_MESSAGES.INVALID_TOKEN,
        401,
