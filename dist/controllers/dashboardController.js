...
const getTopProducts = asyncHandler(async (req, res) => {
    ...
    async function getProductStats(start, end) {
        const orders = await Order.find({ createdAt: { $gte: start, $lte: end } });
        const productMap = {};
...
const getCustomReport = asyncHandler(async (req, res) => {
    ...
    if (filters.product)
        match["orderItems.product"] = filters.product;
    if (filters.customer)
        match["user"] = filters.customer;
    if (filters.status)
    ...
    if (metrics.includes("revenue"))
        group.revenue = { $sum: "$totalAmount" };
    if (metrics.includes("orders"))
        group.orders = { $sum: 1 };
    if (metrics.includes("unitsSold"))
        group.unitsSold = { $sum: { $sum: "$orderItems.qty" } };
    ...
    if (dimensions.includes("product")) {
        const productIds = results.map((r) => r._id.product).filter(Boolean);
    ...
    if (dimensions.includes("customer")) {
        const customerIds = results.map((r) => r._id.customer).filter(Boolean);
