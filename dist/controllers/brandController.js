...
// ⏩ queue producers
import { enqueueAdminBrandAdded, enqueueAdminBrandUpdated, enqueueAdminBrandDeleted, } from '../queues/producers/adminNotificationProducers.js';
...
const updateBrand = asyncHandler(async (req, res) => {
    ...
    const changes = [];
    if (updatedBrand) {
        ['name', 'description', 'website', 'country', 'isActive', 'logo'].forEach((k) => {
            if (before[k] !== updatedBrand[k])
                changes.push(k);
        });
    }
